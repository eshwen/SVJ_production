...
if [ -z $1 ]; then
  echo "-----------------------------------------------------------------------------------------------------------------------------
Usage: ./set_batch.sh  WORKING_DIRECTORY NUMBER_OF_EVENTS  ALPHA_D M_Z R_INV SEED NUMBER_OF_THREADS(to not execute cmsRun leave empty)
-----------------------------------------------------------------------------------------------------------------------------"
  exit
fi
...
$PWD/set_config.sh $workdir $n_of_events $alpha_D $m_Z $r_inv $seed $n_of_threads 
